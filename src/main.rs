...
fn random_walk(rng: &mut ThreadRng, p: f64, n: usize) -> Vec<i64> {
    let bern: Bernoulli = Bernoulli::new(p).unwrap();
    bern.sample_iter(rng)
        .take(n)
        .scan(0, |state, x| {
...
fn random_walk_end(rng: &mut ThreadRng, p: f64, n: usize) -> i64 {
    let bern: Bernoulli = Bernoulli::new(p).unwrap();
    bern.sample_iter(rng)
        .take(n)
        .fold(0, |acc, x| acc + (2 * (x as i64) - 1))
}
...
fn main() {
    ...
    let walkers: usize = 100000;

    // collect the results into a vector:
    let rw_end: Vec<i64> = (0..walkers).map(|_| random_walk_end(&mut rng, 0.5, steps)).collect();
    ...
    let rw_end_dist: Vec<i64> = rw_end.into_iter().fold(vec![0; (max - min + 1) as usize], |mut state, x| 
        {
